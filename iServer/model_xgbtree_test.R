...
##
data <- reactive({
  req(input$mxgbtt_upload)
  dsb <- read.csv(input$mxgbtt_upload$datapath, header = TRUE, stringsAsFactors = FALSE)
})

output$mxgbtt_ex_cols_ui <- renderUI({
  list(
    selectInput("mxgbtt_ex_cols", label = NULL, 
                choices = colnames(data()),
                multiple = TRUE, selectize = TRUE, 
                selected = NULL),
    actionButton("mxgbtt_ex_cols_select", "Select", width = blotter_field_default_width)
  )
})

observeEvent(input$mxgbtt_run_save, {
  ...
  if(input$xgbt_bs_save_res == "y"){
    CreateDirIfNotExist(paste0("Output/Prediction"))
    write.csv(tuning_res[[1]], file = paste0("Output/Prediction/xgbTree_grid_",
