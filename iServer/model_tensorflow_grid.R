...
##
observeEvent(input$tf_gs_search, {
    ...
    detail = 'This may take a while ...', value = 0, {
      tuning_res <- tryCatch({
        gr <- GridSearchTF2(
          ...
          save_pred = ifelse(input$tf_gs_save_pred == "y", TRUE, FALSE),
          save_model = ifelse(input$tf_gs_save_mod == "y", TRUE, FALSE)
        )
  ...
  if(input$tf_bs_save_res == "y"){
    ...
    write.csv(tuning_res[[1]], file = paste0("Output/Text/tensorflow_grid_",
  ...
  output$tf_gs_search_result <- DT::renderDataTable({
    DT::datatable(
      tuning_res[[1]], 
      options = list(
        pageLength = 10,
  ...
  output$tf_gs_message <- renderText({
    tuning_res[[2]]
  })
  
  # step 6. output Search graphs
  output$tf_gs_graph <- renderUI({
    list(
      textInput("tf_gs_graphx", NULL, value = paste0(input$cgen_root_dir, input$cgen_proj_name, "/Output/"),
