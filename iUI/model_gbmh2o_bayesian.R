tp_model_gbmh2o_bayesian <- tabPanel(
  "Bayesian Search",
  fluidRow(
    column(
      ...
      lapply(1:nrow(bs_pars), function(i){
        numericInput(paste0("mgbmb_", bs_pars[i, "par"]), label = bs_pars[i, "desc"],
      ...
      lapply(1:nrow(unv_pars), function(i){
        radioButtons(paste0("mgbmb_", unv_pars[i, "par"]), unv_pars[i, "desc"], 
                     choices = c(unv_pars[i, "choice1"], unv_pars[i, "choice2"]), 
                     selected = unv_pars[i, "default"])
      }),

      actionButton("mgbmb_run", "Run"),
      textOutput("mgbmb_run_msg")
    ),
    column(
      width = 12 - tp_wid_nar,
      tags$div(
        ...
        tags$div(class = "content_wrapper", DT::dataTableOutput("mgbmb_sb"))
      )
    )
  )
)