  ...
  fluidRow(
    column(
      ...
      lapply(1:nrow(unv_pars), function(i){
        radioButtons(paste0("mtfg_", unv_pars[i, "par"]), unv_pars[i, "desc"], 
                     choices = c(unv_pars[i, "choice1"], unv_pars[i, "choice2"]), 
                     selected = unv_pars[i, "default"])
      }),

      actionButton("mtfg_run", "Run"),
      textOutput("mtfg_run_msg")
    ),
    column(
      width = 12 - tp_wid_nar,
      tags$div(
        ...
        tags$div(class = "content_wrapper", DT::dataTableOutput("mtfg_sb"))
      ),
      br(),
      tags$div(
        class = "frame_wrapper",
        tags$div(class = "title_wrapper", tags$h5(class = "title_content_lg", " Confusion Matrix")),
        tags$div(class = "content_wrapper", uiOutput("mtfg_cfmtx"))
      ),
      br(),
      tags$div(
        class = "frame_wrapper",
        tags$div(class = "title_wrapper", tags$h5(class = "title_content_lg", " Variable Importance")),
        tags$div(class = "content_wrapper", uiOutput("mtfg_varimp"))
      )
    )
  )
)