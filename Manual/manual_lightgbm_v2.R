...
source("C:/Users/Ke/Desktop/Data Science/ShinyMLHome/global.R")
setwd(proj_dir)
load("hcdr_full04.RData")
#load("lightgbm_5000n_features.RData")
rm(aggr_testdata_ohe_scaled)
gc()

##
# Take validation set
allrows <- 1:nrow(km_tgt)
n_val_size <- 0.1
set.seed(123)
val_idx <- sample(allrows, floor(n_val_size*nrow(km_tgt)), replace = FALSE)
rem_idx <- allrows[!(allrows %in% val_idx)]

rem_target <- km_tgt[rem_idx, , drop = FALSE]
rem_predictors <- aggr_predictors_ohe_scaled$coredata[rem_idx, -1]

val_target <- km_tgt[val_idx, , drop = FALSE]
val_predictors <- aggr_predictors_ohe_scaled$coredata[val_idx, -1]

rm(km_tgt, aggr_predictors_ohe_scaled, allrows)
gc()
...
br <- GridSearchLgbm2(
  proj = "HCDR",
  model_name = "lightgbm",
  dataset = as.matrix(train_sltd),
  labels = tgts,
  job = "bc",
  val_size = floor(length(tgts$StrTarget)/20),
  cv_rep = 1,
...
##
# Predict
val_sltd <- val_predictors[, ]
val_tgts <- val_target$StrTarget
rm(val_predictors)
gc()

val_pred <- predict(br$models[[1]][[1]], as.matrix(val_sltd))
...
if(TRUE){
  ...
  rm(val_sltd, train_sltd)
  load("hcdr_full04.RData")
  rm(aggr_predictors_ohe_scaled)
  gc()
  
  test_sltd <- aggr_testdata_ohe_scaled$coredata[,]
  rm(aggr_testdata_ohe_scaled)
  gc()
  ...
  pred_res <- predict(br$models[[1]][[1]], as.matrix(test_sltd))
