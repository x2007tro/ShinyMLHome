...
##
# Core regression train
##
CoreTrainReg2 <- function(x, y, job = c("bc", "mc", "rg"), target_name = "target"){
  
  # Combine data
  y <- as.data.frame(y)
  colnames(y) <- target_name
  datasetf <- rbind.data.frame(x, y)
  
  # train
  fmr <- as.formula(paste(target_name,"~", paste(colnames(x), collapse="+")))
  if(job == "bc"){
    mdl <- glm(fmr, data = datasetf, family = binomial())
  } else if(job == "mc"){
    mdl <- nnet::multinom(fmr, data = datasetf)    # Needs scale
  } else if (job == "rg"){
    mdl <- lm(fmr, data = datasetf)
  } else {
