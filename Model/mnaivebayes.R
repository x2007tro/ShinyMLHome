...
# GridSearch
##
GridSearchNavBay2 <- function(proj = "",
                               model_name,
                               dataset,
                               labels,
                               job = c("bc", "mc", "rg"),
  ...
  if(nrow(gs_ly_pars) > 0){
    res2 <- lapply(1:nrow(gs_ly_pars), function(i){
      ...
      #
      res <- CrossValNavBay2(proj = gs_proj,
...
CrossValNavBay2 <- function(proj = "",
  ...
  all_res <- lapply(1:cv_cv_rep, function(i){
    ...
    # 4. valdn_result
    #
    mdl <- TrainNavBay2(proj_nm = cv_proj,
                         model_name,
                         split_id = splt_sd,
                         job = cv_jb,
  ...
  cv_res <- cv_res %>% 
    ...
    dplyr::group_by(proj, job, laplace) %>% 
...
TrainNavBay2 <- function(proj_nm = "",
  ...
  ##
  mdl <- CoreTrainNavBay2(x = mdl_trds,
                           y = mdl_trl,
                           pars = mdl_pars,
                           job = mdl_job)
  ##
...
# Core train
##
CoreTrainNavBay2 <- function(x, y, x_val, y_val, pars, 
                              job = c("bc", "mc", "rg")){
  ...
  mdl <- naivebayes::naive_bayes(formula = fmr, data = train_data, laplace = pars[1, "laplace"])
