...
CrossValLgbm2 <- function(proj = "",
  ...
  all_res <- lapply(1:cv_cv_rep, function(i){
    ...
    # aggregation id
    aggr_id_strain <- cv_agr_id[train_idx]
    aggr_id_valdn <- cv_agr_id[valdn_idx]
    aggr_id_holdout <- cv_agr_id[holdout_index]
    ...
    if(cv_jb == "bc"){
      ...
      rmse_res <- GACRPEvaluation(
        visitor_id = aggr_id_holdout,
        proj_rev = ho_pred,
        actual_rev = target_holdout
      )
      ho_res <- rmse_res
    } else {
      ho_res <- 0
